...
config_website(){
	...
	wget https://github.com/xxxbrian/trojan.sh/raw/main/index.zip
	unzip index.zip
...
start_docker(){
	...
	docker run --name trojan --restart=always -d -p 80:80 -p 443:443 -v /usr/src/trojan:/usr/src/trojan  xxxbrian/trojan sh -c "/etc/init.d/nginx start && trojan -c /usr/src/trojan/server.conf"
	echo "============================"
	echo "       trojan启动完成"
	echo "============================"
}
